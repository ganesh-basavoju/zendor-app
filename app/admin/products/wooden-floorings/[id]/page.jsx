...
const ViewProduct = ({ params }) => {
  ...
  useEffect(() => {
    const fetchProduct = async () => {
      try {
        const res = await axiosInstance.get(`/wooden-floors/product/${params.id}`);
        if (res.data.status === "success") {
          ...
          setProductData({
            ...
            runningProfileCost: product.profileCost || '', // Updated to match backend
            perUnit: product.costPerSqFtPerBox?.box || '', // Updated to match backend
            ...
            image: product.images ? { // Updated to match backend
              url: product.images,
              name: 'Product Image'
            } : null,
            imageUrl: product.images || '' // Updated to match backend
          });
        }
      } catch (error) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const updatedData = {
        ...
        costPerSqFtPerBox: { box: Number(productData.perUnit) },
      ...
      const res = await axiosInstance.put(`/wooden-floors/${params.id}`, updatedData);
      if (res.data.status === "success") {
  ...
  const handleDelete = async () => {
    if (window.confirm('Are you sure you want to delete this product?')) {
      try {
        const res = await axiosInstance.delete(`/wooden-floors/${params.id}`);
        if (res.data.status === "success") {
          router.push('/admin/categories/wooden-floorings');
        }
      } catch (error) {
  ...
  return (
    <form onSubmit={handleSubmit} className="min-h-screen bg-gray-50 p-4 md:p-6">
      ...
      <div className="flex items-center gap-4 mb-8">
        ...
        <div>
          <h1 className="text-2xl md:text-3xl font-semibold text-[#003f62]">Edit Wooden Flooring Item</h1>
        </div>
      </div>
      ...
      <div className="flex gap-2">
        ...
        <div className="relative">
          ...
          <label className="block text-sm font-medium text-gray-700 mb-2">Note: If you upload new image ,it replaces current image </label>
